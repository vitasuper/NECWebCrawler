...
import com.longyue.crawler.Model.WebPage;
import com.longyue.crawler.Model.WebPage.PageType;
...
public class HtmlParser {
    ...
    public List<WebPage> parsePlaylists(String url) {
        ...
        return playLists.stream().map((e) -> {return new WebPage(BASE_URL + e.attr("href"), PageType.playlist);}).collect(Collectors.toList());
    }

    public List<WebPage> parsePlaylist(String url) {
        Document document = Jsoup.parse(HTML_FETCHER.fetch(url));
        Elements playList = document.select("ul.f-hide li a");  // neater
        return playList.stream().map((e) -> {return new WebPage(BASE_URL + e.attr("href"), PageType.song);}).collect(Collectors.toList());

//        // Old version: 17/10/12. No need to be this complicated.
//        Document doc = Jsoup.parse(HTML_FETCHER.fetch(url));
//        Elements playList = doc.select(".f-hide");
//        List<WebPage> result = new ArrayList<>();
//        for (Element e: playList) {
//            Elements liSet = e.getElementsByTag("li");
//            for (Element li: liSet) {
//                Elements aSet = li.getElementsByTag("a");
//                for (Element a: aSet) {
//                    result.add(new WebPage(BASE_URL + a.attr("href"), PageType.song));
//                }
//            }
//        }
//        return result;
    ...
    public static <T> void main(String[] args) throws Exception {
        ...
        htmlParser.parsePlaylists("http://music.163.com/discover/playlist/?order=hot&cat=%E5%85%A8%E9%83%A8&limit=35&offset=0").forEach(playlist -> System.out.println(playlist));
        System.out.println("=====================");
